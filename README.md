...
- `POST /api/pick` — body `{ "file": "Material Symbols/<name>.svg" }`，把该 SVG 拷贝到 `Picked/`（同名直接覆盖）。
...
- **备选目录**：详情条中每个形态（填充 / 描边）都有独立的「添加至备选」按钮，点击把对应 SVG 拷贝到项目根目录下的 `Picked/`（同名覆盖）；右侧「打开备选目录」按钮在系统资源管理器中打开该目录。
